...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-summit-sec-token-detail.ps1

php tests/test_domain_review.php

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-summit-sec-token-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-summit-sec-token-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-summit_sec_token_lens-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
